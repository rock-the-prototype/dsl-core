...
Deno.test("accepts canonical 'As <actor>, I must ...' form", () => {
    ...
    if (atom.modality !== "must") throw new Error(`modality mismatch: ${atom.modality}`);
    if (atom.action !== "validate the access token") throw new Error(`action mismatch: ${atom.action}`);
});
...
Deno.test("rejects 'As <actor>, must ...' missing subject after 'As'", () => {
    ...
    if (!threw) throw new Error("Expected parseRequirement to throw for missing subject after 'As <actor>,'");
});