...
export async function cmdValidate(
  ...
  const hasParserErrors = files.some((f) => f.statements.some((s) => !!s.error));
  ...
  if (opts.json) {
    ...
    for (const f of files) {
      const invalid = f.statements.filter((s) => s.validation && !s.validation.valid).length;
      const parseErr = f.statements.filter((s) => s.error).length;
      const ok = f.statements.length - invalid - parseErr;
      console.log(`${f.path}: OK=${ok} INVALID=${invalid} PARSE_ERR=${parseErr}`);
    }
  }
