...
This folder contains **public JSON Schema contracts** for `dsl-core`.
These contracts define the **stable, machine-consumable outputs** of the engine (e.g., validation and audit reports).
...
- How downstream tools (CI, orchestrators, external orgs) should consume these contracts
...
Internal engine structures live under `src/model/**` (formerly `src/schema/**`) and are **not contracts**.
...
Downstream tools should be able to **pin to a schema version** and rely on deterministic behavior.
...
> The engine should always be able to emit a report that conforms to the schema version it declares.

---

## Consumption & pinning

Downstream tooling should pin **exact versions** (tag/commit) for reproducibility.
...
Rule of thumb:
> Internal models may change for correctness and maintainability.
> Public contracts only change when necessary. 
> Every single artefact always evoles with clear versioning.
...
2. Update or add examples in `examples/**` (if present) so consumers can validate.
